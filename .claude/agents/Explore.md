---
name: Explore
description: Read-only search agent for broad fan-out searches. Use proactively when it's necessary to sweep many files, directories, or naming conventions and you only need the conclusion. Reads excerpts rather than whole files. Specify search breadth with "brief" for quick checks, "medium" for moderate exploration, and "very thorough" for multiple locations and naming conventions.
