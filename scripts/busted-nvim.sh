...
#
# LUA_PATH/LUA_CPATH point at the user's Lua 5.1 luarocks tree: Neovim's
# LuaJIT speaks the 5.1 ABI, so busted must be installed with
# `luarocks --lua-version=5.1 install busted` (the default homebrew tree is
# Lua 5.5 — its C modules cannot load into LuaJIT).
#
